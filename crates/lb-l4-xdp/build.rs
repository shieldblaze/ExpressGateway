//! Detect whether scripts/build-xdp.sh has produced src/lb_xdp.bin and expose that via the
//! `lb_xdp_elf` cfg, so the loader can `include_bytes!` it without a hard file dependency.
//!
//! EBPF-2-01: the committed ELF is the load-bearing artefact, so a 64 KiB ceiling is enforced here
//! to stop unbounded size drift. `tests/elf_sections.rs` re-asserts the same invariant.
...
fn main() {
    ...
    if let Ok(meta) = std::fs::metadata(&elf_path) {
        ...
        if size > MAX_ELF_BYTES {
            // Refuse to emit the cfg: an oversized ELF means the eBPF source grew without a
            // deliberate ceiling bump, and the proof test would reject it anyway.
            panic!(
