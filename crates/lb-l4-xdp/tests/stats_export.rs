//! EBPF-2-08 proof test: the STATS per-CPU array export API. Pins the wire-stable slot-index
//! values, the `HandleMissing` (never-panic) behaviour before `install_stats_handle`, and the
//! snapshot shape.
...
fn slot_indices_match_ebpf_constants() {
    // Re-derive from the eBPF source (the `STAT_*` constants) so editing the eBPF crate without
    // touching the userspace enum is caught.
...
fn read_stats_without_install_returns_handle_missing() {
    // No handle installed in this process. The contract is: never panic, always a Result.
    // (cargo runs each test file in its own binary, so cross-binary contamination is impossible.)
    let r = read_stats();
    match r {
...
/// Wire-stability guard for the STATS slot count (currently 16). Each bump MUST come with (a) a
/// new `STAT_*` constant in the eBPF crate, (b) a new `StatSlot` appended at the END, and (c)
/// this assertion updated in lock-step.
#[test]
fn num_slots_constant_tracks_appended_slots() {
    assert_eq!(NUM_SLOTS, 16);
}

/// EBPF-2-08 named proof test for the kernel-side path.
#[test]
