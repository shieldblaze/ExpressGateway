...
fn read_stats_without_install_returns_handle_missing() {
    // No handle installed in this process. The contract is: never panic, always a Result. (cargo
    // runs each test file in its own binary, so cross-binary contamination is impossible.)
    let r = read_stats();
    match r {
...
/// Wire-stability guard for the STATS slot count (currently 16). Each bump MUST come with a new
/// `STAT_*` constant in the eBPF crate, a new `StatSlot` appended at the END, and this assertion
