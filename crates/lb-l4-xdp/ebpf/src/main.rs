...
//!
//! IPv4: parse eth (optionally stripping ONE 802.1Q tag — QinQ is deferred), ACL_DENY_TRIE on
//! src IP, ports, CONNTRACK; on a hit rewrite MAC + dst IP + dst port with RFC 1624
//! incremental checksum updates and `XDP_TX`, on a miss `XDP_PASS` so userspace picks a
//! backend. IPv6 mirrors it (up to two extension headers; no L3 checksum, but the L4 checksum
...
// EBPF-2-01/02: the kernel `BPF_PROG_LOAD` syscall reads `bpf_attr.license` from this ELF
// section. Declaring it explicitly removes a dependency on aya-obj's "GPL" default, and
// `no_mangle` keeps bpf-linker's DCE from stripping the symbol.
...
use aya_ebpf::{
    bindings::xdp_action,
    macros::{map, xdp},
    // EBPF-2-03: CONNTRACK/CONNTRACK_V6 are LruHashMap so the kernel evicts the oldest entry
    // under flood instead of returning ENOMEM. L7_PORTS is config-managed, never
    // flood-pressured, so it stays a plain HashMap.
...
/// IPv6 extension-header shape; only `next_header` + `hdr_ext_len` are consulted.
/// `hdr_ext_len` is in 8-byte units NOT counting the first 8 bytes — the kernel convention.
#[repr(C, packed(2))]
struct Ipv6ExtHdr {
    next_header: u8,
    hdr_ext_len: u8,
...
/// `bpf/lib/conntrack.h`). Pure LRU is vulnerable to a sliding-RST replay: an adversary
/// spraying RST/FIN across evicted flows fills the LRU's young end and pushes live flows out.
/// Pruning on RST and on FIN-ACK tracks TCP-FSM reality without a verifier-costly full FSM.
...
// ROUND8-L4-04: atomic per-VIP backend-table publication (Unimog / l4drop D1 lesson).
// `BackendTable` is ONE map value, so a single `bpf_map_update_elem` of the whole struct is
// atomic w.r.t. concurrent lookups — a reader sees the entire old or entire new table, never a
// torn merge. That single-syscall publication IS the fix for the previous N-syscall window.
//
...
/// ROUND8-L4-04: verifier-tractable ceiling on backends per VIP. More needs partitioning or
/// Maglev consistent hashing (`audit/deferred.md`).
pub const MAX_BACKENDS_PER_VIP: usize = 64;

/// ROUND8-L4-04: per-VIP backend table, published atomically as a single map value (Unimog
/// D1). `generation` increments on every publish; a CT entry whose remembered generation
/// differs is in the transitional window and consults `previous_entries` (Unimog lesson 3) so
/// in-flight flows reach the previous backend instead of being stranded.
#[repr(C)]
#[derive(Clone, Copy)]
...
/// ROUND8-L4-04: verifier-safe, behaviorally-inert reference to `BACKENDS_V4`, called once on
/// the IPv4 CT-miss path so (1) bpf-linker DCE keeps the map + its BTF alive and (2) a lookup
/// that observes a published table proves the single-syscall swap is visible to the data plane.
///
/// It deliberately does NOT select a backend or mutate CONNTRACK — that is the deferred
...
/// ROUND8-L4-01: a conntrack hit whose backend_ip/backend_port is zero is an unpopulated
/// controller entry. PASS (not drop) so the kernel still routes it; the counter is the signal.
const STAT_BACKEND_UNPOPULATED: u32 = 10;
/// ROUND8-L4-08: IPv4 fragment seen — passed to the kernel for reassembly (Katran/Cilium
/// design: no in-XDP reassembly).
const STAT_V4_FRAGMENT: u32 = 11;
const STAT_V6_FRAGMENT: u32 = 12;
/// ROUND8-L4-02: a TCP RST evicted its conntrack entry. The packet still goes `XDP_PASS` so
/// the RST reaches the peer end-to-end; only flow *tracking* stops.
...
/// `is_under_flood()`, above the per-CPU cap the miss path short-circuits to `XDP_PASS`
/// WITHOUT signalling userspace to populate CONNTRACK, so the LRU stays stable for established
/// (CT-hit) flows instead of being thrashed by the attacker's unique 5-tuples.
const STAT_NEW_FLOW_RATE_CAP: u32 = 15;

// EBPF-2-03: LRU_HASH evicts the oldest entry under flood instead of returning ENOMEM at
// insert, closing the flow-spray DoS that starved legitimate new connections.
//
// EBPF-2-05: the explicit lowercase `name = …` decouples the on-disk pin filename from Rust
// identifier churn — aya defaults it to the uppercased identifier, so a rename of the Rust
// static would force a pin rename + state loss.
#[map(name = "conntrack")]
...
/// IPv4 deny ACL as a longest-prefix-match trie. Key data is the address in network byte
/// order; `prefix_len` is the CIDR mask length.
...
// ROUND8-L4-03: per-CPU new-flow-rate tracker (Katran `is_under_flood()` lesson 4). Under a
// SYN flood an attacker sprays millions of unique 5-tuples/sec; each is a CT miss the control
// plane would answer with a `bpf_map_update_elem`, making every established flow an LRU
// eviction loser. The cap is per-CPU (XDP runs one instance per RX queue, so no cross-CPU
...
/// Runtime-tunable per-CPU new-flow cap — a one-entry per-CPU array the hot path reads once
/// per CT-miss (verifier-cheap). A zero value means the operator DISABLED the cap.
#[map(name = "new_flow_cap_cfg")]
static NEW_FLOW_CAP_CFG: PerCpuArray<u32> = PerCpuArray::<u32>::with_max_entries(1, 0);

/// ROUND8-L4-03: Katran `MAX_CONN_RATE` parity — the compile-time fallback for the window
/// before userspace first writes `NEW_FLOW_CAP_CFG`. Since 0 in the cfg map means "operator
/// disabled", not-yet-written is distinguished from disabled by consulting this fallback ONLY
/// when the slot is unreadable.
const DEFAULT_NEW_FLOW_CAP_PER_CPU: u32 = 125_000;

const RATE_WINDOW_NS: u64 = 1_000_000_000;

/// ROUND8-L4-03: true when this CPU has admitted more than the configured cap of new flows in
/// the current 1 s window (Katran `is_under_flood()`: per-CPU window, reset on rollover,
...
// documented scalar/pointer re-ordering on recent rustc/LLVM. This guards the
// CVE-2022-23222-class bounds-check elision for any future runtime-controlled offset;
// `checked_add` lowers to `llvm.uadd.with.overflow.i64`, which the verifier handles on 5.15+.
//
...
// RFC 1624 incremental checksum helpers. §3: HC' = ~(~HC + ~m + m'), where HC is the old
// checksum, m the old 16-bit field and m' the new one. Operates on already-folded
...
fn handle_ipv4(ctx: &XdpContext, l3_offset: usize) -> Result<u32, ()> {
    ...
    // ROUND8-L4-08: RFC 791 §3.1 — `frag_off` bit 14 is MF and bits 0..12 are the offset in
    // 8-byte units. MF==1 or offset>0 means this is not a complete datagram: pass to the kernel
    ...
    // LPM key data is u32 in network byte order; a /32 lookup returns the most specific
    // matching deny prefix.
    ...
    // ROUND8-L4-02: prune BEFORE the lookup-and-rewrite hot path. The LRU returns the entry
    // even on RST/FIN-ACK and we always want the slot freed, so a sliding-RST replay cannot pin
    // LRU capacity. The packet goes XDP_PASS on RST and XDP_TX on FIN-ACK (the last FIN-ACK
    // still needs rewriting and forwarding).
    ...
    let entry: BackendEntry = match unsafe { CONNTRACK.get(&key) } {
        Some(v) => *v,
        None => {
            // ROUND8-L4-04: behaviorally-inert touch that keeps the map + BTF alive for
            // userspace `publish_backends_v4` and proves the publication is visible here.
            let _table_ready = backend_table_published(dst_addr);
            // ROUND8-L4-03: a CT miss is a NEW flow — the attacker's lever under a SYN flood.
            // Above the per-CPU cap, short-circuit to XDP_PASS WITHOUT the STAT_PASS signal the
            // control loop reads as "populate CT". CT-hit flows above are unaffected.
            if is_under_flood() {
...
fn handle_ipv6(ctx: &XdpContext, l3_offset: usize) -> Result<u32, ()> {
    ...
    // ROUND8-L4-08: the IPv6 Fragment Extension Header (RFC 2460 §4.5) is present in BOTH first
    // and later fragments, so any packet carrying it lacks a complete L4 header to rewrite.
