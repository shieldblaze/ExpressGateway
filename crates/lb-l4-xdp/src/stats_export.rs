//! Lock-step API boundary between the eBPF data plane and userspace observability. Everything here is safe, lock-free and panic-free at steady state — telemetry must never be the reason production aborts.
use std::sync::atomic::{AtomicU8, Ordering};

// EBPF-2-08: the per-CPU STATS surface is Linux-only because aya is. The label / pin-reused / slot-enum APIs are pure Rust and stay available everywhere.
#[cfg(target_os = "linux")]
...
impl AttachModeLabel {
    ...
    /// Prometheus label value, matching the kernel API vocabulary so an operator can compare it against `bpftool net show`.
    #[must_use]
...
/// Process-global atomic store of the current XDP attach mode. Single producer (the startup attach path), many consumers; at most one XDP attach per process.
...
/// Read back the current attach mode. `None` when XDP has not been attached, so the gauge reports 0 for every mode rather than fabricating a value.
#[must_use]
pub fn current_attach_mode() -> Option<AttachModeLabel> {
    AttachModeLabel::from_byte(ATTACH_MODE.load(Ordering::Relaxed))
}

/// Process-global count of `Drv` attaches the blocklist refused or the probe found dead, forcing a demotion to `Skb`. Userspace-only — the BPF program never runs when an attach silently drops, so there is no kernel `STATS` slot.
...
/// Read back the cumulative attach-probe-failed count for Prom exposition (`xdp_attach_probe_failed_total`).
#[must_use]
...
/// Canonical pin-name ordering for the bitmap. Append to the END only — bit positions are wire-stable.
#[must_use]
...
/// Record whether the named pin was reused. Unknown names are silently dropped (forward compatibility with future pin additions).
...
/// Slot indices into the eBPF program's `STATS: PerCpuArray<u64>`. MUST stay in lock-step with the `STAT_*` constants in `ebpf/src/main.rs`; order is wire-stable, so append only.
...
pub enum StatSlot {
    ...
    V6ExtUnsupported = 9,
    /// `STAT_BACKEND_UNPOPULATED` (ROUND8-L4-01): a conntrack hit whose `backend_ip == 0` or `backend_port == 0` — controller wrote an unpopulated entry.
    BackendUnpopulated = 10,
    /// `STAT_V4_FRAGMENT` (ROUND8-L4-08): IPv4 packet with MF set or fragment offset > 0.
    V4Fragment = 11,
    /// `STAT_V6_FRAGMENT` (ROUND8-L4-08): IPv6 packet carrying a Fragment Extension Header (IPPROTO_FRAGMENT = 44).
    V6Fragment = 12,
    /// `STAT_CT_RST_PRUNE` (ROUND8-L4-02): a TCP RST packet evicted its conntrack entry (Cilium `bpf/lib/conntrack.h` RST-prune lesson).
    CtRstPrune = 13,
    /// `STAT_CT_FIN_PRUNE` (ROUND8-L4-02): a TCP FIN-ACK packet evicted its conntrack entry.
    CtFinPrune = 14,
    /// `STAT_NEW_FLOW_RATE_CAP` (ROUND8-L4-03): a new flow was rate-capped under a SYN flood (Katran `is_under_flood()`). The userspace `CtInsertGate` increments the SAME slot when it denies a control-plane CT insert.
    NewFlowRateCap = 15,
    /// `xdp_attach_probe_failed_total` (ROUND8-L4-05): the blocklist or probe found the requested `Drv` attach dead and the loader demoted to `Skb`. NOT a kernel per-CPU slot — no `STAT_*` constant, surfaced via [`attach_probe_failed_count`], never `read_stats()`, but it holds a wire-stable position so the slot vocabulary stays single-sourced.
    AttachProbeFailed = 16,
}

/// Number of KERNEL-side per-CPU `STATS` slots — the length of the `read_stats()` lookup loop. A bump MUST come with a new `STAT_*` constant in the eBPF crate AND a new [`StatSlot`].
///
/// ROUND8-L4-05: `StatSlot::AttachProbeFailed` (16) is deliberately NOT counted — it is userspace-only, and keeping `NUM_SLOTS == 16` is what bounds the kernel read loop to real kernel slots.
...
/// Owned snapshot of the STATS map at one moment. `summed[i]` is the cross-CPU sum (all the Prom scraper publishes); `per_cpu[i]` is the un-summed slice for the debug endpoint.
#[derive(Debug, Clone)]
...
/// Install the STATS map handle, once per process. EBPF-2-08 invariant: the typed wrapper is cached but the VALUES never are — each `PerCpuArray::get` is a fresh `bpf_map_lookup_elem`, so the scraper always sees live state.
#[cfg(target_os = "linux")]
...
/// Read a fresh STATS snapshot — the public Prom-side entry point. Cost: one `bpf_map_lookup_elem` per slot per scrape; on a 256-CPU host each syscall returns 2 KiB, so a scrape copies ~20 KiB of kernel memory.
#[cfg(target_os = "linux")]
...
/// Non-Linux stub returning zeros sized to [`NUM_SLOTS`], so cross-platform consumers need no `cfg` gates.
...
mod tests {
    ...
    fn pin_reuse_unknown_name_is_silent() {
        // Forward-compat: a pin name added in the eBPF crate but not yet in `pin_names()` must not panic.
        record_pin_reused("future_map", true);
    }

    #[test]
    fn stat_slot_indices_are_wire_stable() {
        // Wire-stability invariant: each slot's numeric value is published to operators via `xdp_packets_total{result}` labels; reordering breaks Prom recording rules.
        ...
        // ROUND8-L4-05: userspace-only slot (NOT counted in NUM_SLOTS — see the const doc). Wire position is still stable so the exposition vocabulary is single-sourced.
    ...
    fn num_slots_matches_enum() {
        // A new StatSlot variant without a NUM_SLOTS bump would be silently skipped by the `read_stats` read loop — this assertion guards that.
