//! Lock-step API boundary between the eBPF data plane and userspace observability.
//!
//! Everything here is safe, lock-free and panic-free at steady state — telemetry must never be
//! the reason production aborts. File ownership: the ebpf side owns this file; consumers read
//! it through the `pub fn` accessors and MUST NOT edit it.
...
impl AttachModeLabel {
    ...
    /// Prometheus label value, matching the kernel API vocabulary so an operator can compare
    /// it against `bpftool net show`.
    #[must_use]
...
/// Process-global atomic store of the current XDP attach mode. Single producer (the startup
/// attach path), many consumers; there is at most one XDP attach per process.
...
/// Read back the current attach mode. `None` when XDP has not been attached, so the gauge
...
/// Process-global count of `Drv` attaches the blocklist refused or the probe found dead,
/// forcing a demotion to `Skb`. Userspace-only — the BPF program never runs when an attach
/// silently drops, so there is no kernel `STATS` slot.
...
/// Snapshot of which pinned maps were reused vs. freshly created at startup.
///
/// Bit `i` is `1` if the `i`-th pin in [`pin_names()`] was reused. The packing is deliberate:
/// the Prom scrape is a single atomic load projected to per-name gauges, no Mutex.
...
/// Slot indices into the eBPF program's `STATS: PerCpuArray<u64>`. MUST stay in lock-step with
/// the `STAT_*` constants in `ebpf/src/main.rs`; order is wire-stable, so append only.
...
pub enum StatSlot {
    ...
    /// (Katran `is_under_flood()`). Both the operator's SYN-flood alarm and the back-pressure
    /// signal the control loop polls; the userspace `CtInsertGate` increments the SAME slot
    /// when it denies a control-plane CT insert.
    NewFlowRateCap = 15,
    /// `xdp_attach_probe_failed_total` (ROUND8-L4-05): the blocklist or probe found the
    /// requested `Drv` attach dead and the loader demoted to `Skb`. NOT a kernel per-CPU slot —
    /// it has no `STAT_*` constant and is surfaced via [`attach_probe_failed_count`], never
    /// `read_stats()`. It occupies a wire-stable position so the slot vocabulary stays single-
    /// sourced.
    AttachProbeFailed = 16,
}

/// Number of KERNEL-side per-CPU `STATS` slots — the length of the `read_stats()` lookup loop.
/// A bump MUST come with a new `STAT_*` constant in the eBPF crate AND a new [`StatSlot`].
///
/// ROUND8-L4-05: `StatSlot::AttachProbeFailed` (16) is deliberately NOT counted — it is
/// userspace-only, and keeping `NUM_SLOTS == 16` is what bounds the kernel read loop to real
/// kernel slots.
...
/// Owned snapshot of the STATS map at one moment. `summed[i]` is the cross-CPU sum (all the
/// Prom scraper publishes); `per_cpu[i]` is the un-summed slice for the debug endpoint.
#[derive(Debug, Clone)]
...
/// Install the STATS map handle, once per process.
///
/// EBPF-2-08 invariant: the typed wrapper is cached but the VALUES never are — each
/// `PerCpuArray::get` is a fresh `bpf_map_lookup_elem`, so the scraper always sees live state.
#[cfg(target_os = "linux")]
...
///
/// Cost: one `bpf_map_lookup_elem` per slot per scrape. On a 256-CPU host each syscall returns
/// 2 KiB, so a scrape copies ~20 KiB of kernel memory.
#[cfg(target_os = "linux")]
...
/// Non-Linux stub returning zeros sized to [`NUM_SLOTS`], so cross-platform consumers need no
/// `cfg` gates.
...
mod tests {
    ...
    fn pin_reuse_unknown_name_is_silent() {
        // Forward-compat: a pin name added in the eBPF crate but not yet in `pin_names()` must
        // not panic.
