//! ROUND8-L4-05: known-bad NIC + firmware blocklist for native (`Drv`) XDP, plus the post-attach
//! silent-drop probe scaffold (Linux-only — sysfs/ethtool).
//!
//! The hazard (aya #1193, Cilium lesson 8): on some MLX5/ConnectX firmware, DRV mode silently drops
//! — `bpf_link_create` returns success, every map op reports success, and the packet path
//! goes to /dev/null. Two layered defences: a static `(driver, firmware)` blocklist (wired today,
//! best-effort, goes stale) and a runtime `BPF_PROG_TEST_RUN` probe (the real backstop,
...
/// Outcome of [`probe_xdp_silent_drop`]. `ProbeUnavailable` means the probe could not run at all
/// (aya API blocker): the caller MUST treat it as inconclusive, KEEP the attach, and lean on the
/// static blocklist plus the `xdp_attach_probe_failed_total` alert.
...
pub enum DrvSupport {
    /// No blocklist match — `Drv` may be attempted (the runtime probe is still the backstop once it
    /// is reachable).
    Allowed,
...
/// Errors from the sysfs / ethtool introspection path. A read failure is NOT fatal — the caller
/// treats it as "could not determine", allows `Drv`, and relies on the probe + alert.
#[derive(Debug, thiserror::Error)]
pub enum NicCompatError {
    /// `/sys/class/net/<iface>/device/driver` could not be read (interface gone, virtual device
    /// with no driver symlink, ...).
...
/// A single blocklist row. Only `firmware < first_safe` is ever compared, so the row carries the
/// first KNOWN-GOOD version string plus a human reason.
...
struct BlockRow {
    ...
    /// firmware-only key permanently dead and fail-OPEN). `None` = firmware-only row.
...
const BLOCKLIST: &[BlockRow] = &[
    ...
    BlockRow {
        ...
        // F-COR-7: ena never reports firmware via `ethtool -i` on AWS, so the firmware key alone is
        // permanently dead. The row's documented condition is "pre-2024 kernels" and 6.7 (Jan 2024)
        // is the
        // first 2024 mainline line: kernel < 6.7 with unresolved firmware IS the known-bad combo,
        // while kernel >= 6.7 stays Allowed so there is no fleet-wide native-XDP regression.
...
/// F-COR-7: `(major, minor)` of the running kernel, or `None` if unresolvable. Used ONLY as the
/// driver+kernel fallback key when firmware is unresolved on a row with `bad_kernel_below`.
fn current_kernel_mm() -> Option<(u8, u8)> {
    let kv = aya::util::KernelVersion::current().ok()?;
    // KernelVersion fields are crate-private; reconstruct (major, minor) from its public
    // LINUX_VERSION_CODE-style `code()` (code = (major<<16) | (minor<<8) | patch, patch
    // clamped 0..=255).
    let code = kv.code();
...
/// Parse a dotted-numeric version into a comparable `Vec<u64>`. Trailing non-numeric junk (e.g.
/// `16.32.1010 (MT_0000000080)`) is truncated; missing components compare as 0.
...
/// Decide whether `Drv` is safe for `(driver, firmware)` against the static blocklist. Pure, so it
/// is unit-testable without a real NIC.
#[must_use]
...
/// F-COR-7: driver + kernel classification used ONLY when firmware is UNRESOLVED; [`classify`]
/// itself stays pure and UNCHANGED. For a driver carrying a `bad_kernel_below`, an unresolved
/// firmware must NOT silently fail-open — that was the dead path. Kernel below the boundary = the
/// known-bad combo → Refuse; at/above = Allowed. Drivers without the field keep fail-open.
#[must_use]
pub fn classify_unresolved_firmware(driver: &str, kernel: Option<(u8, u8)>) -> DrvSupport {
    for row in BLOCKLIST {
        ...
        let Some(bound) = row.bad_kernel_below else {
            // Firmware-only row with unresolved firmware → fail-open; the probe + alert remain the
            // backstop.
        ...
        return match kernel {
            ...
            // kernel >= boundary → NOT a known-bad combo → Allowed. Kernel unknowable → fail-open:
...
/// Resolve the kernel driver name for `iface` from `/sys/class/net/<iface>/device/driver` (a
/// symlink whose basename is the driver).
...
/// Read the firmware-version line from `ethtool -i <iface>`. A missing `ethtool` binary or a NIC
/// that reports no firmware is NOT fatal — the caller treats it as "could not determine".
...
/// Top-level gate called by `XdpLoader::attach_with_fallback` BEFORE attempting `Drv`. A resolution
/// failure maps to [`DrvSupport::Allowed`] — `Drv` is never blocked merely because introspection
/// failed.
pub fn drv_supported(iface: &str) -> Result<DrvSupport, NicCompatError> {
    ...
    let firmware = match firmware_of(iface) {
        Ok(f) => f,
        // F-COR-7: firmware unresolved. This used to fail-OPEN unconditionally, which made the ena
        // blocklist row permanently dead on real AWS; now it keys on driver+kernel.
        Err(_) => {
...
/// ROUND8-L4-05 runtime probe scaffold: fire a synthetic packet through `BPF_PROG_TEST_RUN` and
/// assert the action is `XDP_TX` AND the dst MAC was rewritten.
///
/// API BLOCKER: aya 0.13.1 exposes no public `test_run` wrapper on the `Xdp` program type, so this
/// returns [`ProbeOutcome::ProbeUnavailable`] and the caller keeps the attach. The static blocklist
/// plus the CI probe fixture and `xdp_attach_probe_failed_total` alert are the active defence until
/// the aya API lands.
#[must_use]
...
mod tests {
    ...
    fn probe_reports_unavailable_on_aya_013() {
        // Documents the API blocker as a behavioural contract: when aya gains a public
        // BPF_PROG_TEST_RUN wrapper this test is the tripwire to wire the real probe.
    ...
    fn ena_unresolved_fw_modern_kernel_stays_allowed() {
        // NOT-known-bad ena box (kernel >= 6.7) with unresolved firmware stays Allowed — native XDP
        // preserved, no fleet regression.
        assert_eq!(
    ...
    fn non_kernel_keyed_driver_unresolved_fw_still_fail_open() {
        // Drivers that report firmware reliably (bad_kernel_below=None) keep the original fail-open
        // on unresolved firmware, as do drivers with no row at all.
        assert_eq!(
    ...
    fn current_kernel_mm_resolves_on_this_box() {
        // aya KernelVersion must resolve a plausible (major, minor) so drv_supported keys correctly
        // on the real path.
