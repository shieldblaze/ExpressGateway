//! Generate the gateway's TOML config + TLS material for each soak datapath. A wrong key silently
//! disables the datapath under test, so the block shapes are pinned here and validated by the
//! apparatus smoke run before any long soak. Mode B is `[listeners.quic.raw_proxy]` and its
//! `backend_ca_path` is MANDATORY for a self-signed backend (the gateway always `verify_peer`s, so
//! omitting it makes the dial fail and the soak would test a dead path); Mode A is the top-level
//! `[passthrough]` block.
...
/// Write a private key with mode 0600 (the gateway's strict TLS-key-permission check rejects
/// group/other-readable keys and exits before binding).
...
/// `idle_timeout_seconds` is generous on purpose: we are proving connection RECLAIM on clean close,
/// not idle-reap, so the sustained echo clients must stay up.
#[must_use]
pub fn h1_front_ws(
    listener: SocketAddr,
    backend: SocketAddr,
...
/// `h1s` front with `[listeners.websocket]` ENABLED **and `h2_extended_connect = true`** -> an H1
/// WebSocket backend (sc8b_ws_h2, RFC 8441). The knob is OFF by default (CF-S27-2), so the soak
...
/// `quic` front in **H3-terminate** mode (no `[listeners.quic.raw_proxy]`, so `raw_quic_backend =
/// None`; R3). This scenario deliberately emits NO backend block, so what it exercises end to
/// end is the `quiche::h3` ingress, the inline-400 DECODED egress, F-MD-4 RST/STOP_SENDING mapping,
/// and the no-backend stream-drop path. The full H3->{H1,H2,H3} relay is covered by the e2e
/// harnesses.
#[must_use]
...
/// `quic` H3-terminate front -> an HTTP/2 gRPC origin (sc9_grpc_h3). Leak-class signal: per-RPC
/// stream open/close plus the response-trailer terminal cleanup (the `drain_resp_channels` path
/// F-S29-1 corrected) under sustained churn — `fds`, RSS/VmHWM, and panic=0.
#[must_use]
...
/// Mode A QUIC passthrough — a top-level `[passthrough]` block; TLS is end-to-end and the gateway
/// never decrypts. `mint_retry = false` is emitted unconditionally: with `true`, the LB-minted
/// Retry trips CF-S15-PASSTHROUGH-RETRY-ODCID and the client is granted 0 streams, so the soak
/// would drive a dead path. `flow_idle_timeout_ms` is the F-S20-2 reaper window, shortened so
/// reclamation is visible within the run.
#[must_use]
...
mod tests {
    ...
    fn quic_h3_terminate_has_quic_listener_no_raw_proxy_no_backend() {
        ...
        // R3 / F-S26-1: an H3-terminate front must carry NEITHER a raw_proxy block (that flips it
        // to Mode B) NOR a backend block.
        assert!(
