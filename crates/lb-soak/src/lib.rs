//! `lb-soak` — ExpressGateway chaos/soak suite. Launches the real `expressgateway` binary as a
//! child, drives co-located load + chaos at it, samples its `/proc` + `/metrics` into a
//! time-series,
//! and computes a BOUNDED/DRIFT verdict (R8). It links NO product crates, so what it measures is
