//! Crate-local QUIC listener lifecycle coverage, closing the finding that
//! `listener.rs` had 0% coverage from the crate's own suite (it was exercised
//! only by repo-root integration binaries that do NOT gate
//! `cargo test -p lb-quic`).
//!
//! Every test spawns the REAL [`lb_quic::QuicListener`] — real UDP bind, real
//! `load_or_generate_retry_secret`, real `build_server_config`, real
//! `router::spawn` — and drives a REAL `quiche` client over loopback, mirroring
//! the proven repo-root harness so it is the same wire path, just gated inside
//! the crate. A single-actor / mis-bind / wrong-ALPN / retry-secret-permission
//! / shutdown-hang regression fails at least one test here.
...
/// Drive a client connection until it is established, closes, or the deadline
/// elapses, returning which terminal state was reached — so callers can assert
/// both the positive and the negative (unknown ALPN must NOT establish) cases.
...
async fn retry_secret_autogenerated_0600_then_reloaded() {
    ...
    // Decisive reload assertion: the reloaded signer must verify a token minted
    // by the FIRST listener's signer, which only holds if the 32-byte secret
    // was loaded back identically. A regression that regenerates fails here.
...
async fn shutdown_cancels_join_cleanly() {
    ...
    // `shutdown()` cancels the token and returns the join handle; it MUST
    // resolve well within budget. A router select! that does not honour
    // `cancel.cancelled()` would hang here.
    let handle = listener.shutdown();
