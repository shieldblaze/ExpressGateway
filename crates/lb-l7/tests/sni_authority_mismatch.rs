//! PROTO-2-15 — validator unit proofs; the hot-path wiring is proven in
//! `tests/sni_authority_421.rs`.
...
fn missing_sni_does_not_falsely_reject() {
    // An SNI-omitting client is valid (RFC 6066 §3); requiring SNI presence is
    // the operator's TLS-config policy, not this validator's.
