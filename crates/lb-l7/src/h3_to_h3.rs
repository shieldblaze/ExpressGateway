//! HTTP/3 to HTTP/3 bridge (pass-through).
//!
//! Both source and destination speak the same pseudo-header scheme,
//! so this bridge performs only lowercase normalization on regular headers.

use crate::{Bridge, BridgeRequest, BridgeResponse, L7Error, Protocol, check_header_count};

/// Pass-through bridge for HTTP/3 to HTTP/3.
pub struct H3ToH3Bridge;

impl Bridge for H3ToH3Bridge {
