...
18. **TEST-002 — `ping_flood_goaway` assertion is best-effort (deferred-with-rationale)**: The PING-flood code path IS exercised end-to-end by `tests/h2_security_live.rs::ping_flood_goaway` — a real h2 client fires a burst of PING frames at the bound listener, hyper's enforcement triggers, the connection tears down. The audit's concern is that the test asserts on `sent > 0` (i.e. some PINGs went out before teardown) rather than asserting on the specific `GOAWAY ENHANCE_YOUR_CALM` error code, because the `h2` 0.4.x client API does not surface the GOAWAY error_code through `SendRequest` after the connection closes — `h2::Error::reason()` returns `None` once the underlying socket is gone. Strengthening this would require either a custom raw-frame reader (large new test infrastructure) or an upstream h2 API change. Both out-of-scope. Per the user's classification rule "code path tested but coverage could be higher → defer", this stays tracked as a known coverage limitation, not a blocker.
