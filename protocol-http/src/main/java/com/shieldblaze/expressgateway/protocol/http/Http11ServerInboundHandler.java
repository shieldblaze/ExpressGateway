...
public class Http11ServerInboundHandler extends ChannelInboundHandlerAdapter implements Closeable {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (msg instanceof HttpRequest request) {
            ...
            String normalizedUri = normalizeUri(request.uri());
    ...
    /**
     * Normalizes a request URI by applying RFC 3986 Section 5.2.4 dot-segment
     * removal to the path component. The query string (if present) is preserved
     * unchanged.
     *
     * <p>In addition to normalization, this method detects path traversal attacks
     * where the number of ".." segments exceeds the depth of real path segments,
     * which would cause the resolved path to escape the document root. The RFC
     * algorithm itself absorbs excess ".." at root (e.g., "/../../etc/passwd"
     * normalizes to "/etc/passwd"), but from a security perspective, a request
     * that attempts to traverse above root is unambiguously malicious and should
     * be rejected. This matches the behavior of Nginx and HAProxy.</p>
     *
     * <p>We intentionally avoid {@link java.net.URI} here because it throws on
     * the malformed URIs that attackers commonly send (e.g., unencoded spaces,
     * bare percent signs). A manual implementation is more robust for a proxy
     * that must handle arbitrary client input.</p>
     *
    ...
    static String normalizeUri(String uri) {
        ...
        // SEC-06: Reject null bytes in path. %00 can cause string truncation in
        // C-based backends (null terminator), enabling smuggling attacks.
        // Check both literal null and percent-encoded form in the path portion only.
        if (path.indexOf('\0') >= 0 || containsIgnoreCase(path, "%00")) {
            return null;
        }

        // SEC-05: Reject double-encoded dots in path. %252e decodes to %2e,
        // which a backend performing a second URL decode would interpret as ".".
        // The sequence %252e%252e would become ".." after double decoding,
        // enabling path traversal. Following Nginx's approach: reject outright.
        if (containsDoubleEncodedDot(path)) {
            return null;
        }

        // LOW-13: Decode percent-encoded dots (%2e/%2E) before path traversal check.
        // Without this, %2e%2e bypasses the dot-segment detection.
        String decodedPath = path;
        if (decodedPath.contains("%")) {
            decodedPath = decodedPath.replace("%2e", ".").replace("%2E", ".");
        }

        // Traversal depth check: walk the path segments and track depth.
        // If ".." ever causes depth to go below zero, the client is trying
        // to escape root. The RFC 3986 algorithm would silently absorb this,
        // but we treat it as an attack.
        if (decodedPath.startsWith("/") && escapesRoot(decodedPath)) {
            return null;
        }

        // GC-02: Fast path — if the decoded path has no '.' characters at all,
        // there are no dot segments to remove and removeDotSegments() would be
        // a no-op. Skip the two StringBuilder allocations on the hot path.
        // This covers the vast majority of production requests (e.g., "/api/v1/users").
     ...
     * by counting segment depth. A ".." that reduces depth below zero means
     * the path attempts to traverse above the root directory.
     *
     * <p>This check is performed on the raw path before RFC 3986 normalization
     * because the normalization algorithm absorbs excess ".." at root level
     * (e.g., "/../../x" becomes "/x"), which would hide the traversal attempt.</p>
     *
     * @param path an absolute path starting with "/"
     * @return {@code true} if the path would escape root, {@code false} otherwise
     */
    static boolean escapesRoot(String path) {
        ...
        while (i < len) {
            ...
            if (segLen == 0) {
                // Empty segment (trailing slash or double slash) — skip
                continue;
    ...
    /**
     * SEC-05: Checks whether the path contains a double-encoded dot sequence.
     *
     * <p>Double-encoded dots take the form {@code %252e} (the {@code %25} is a
     * percent-encoded {@code %}, so {@code %252e} decodes to {@code %2e}, which
     * decodes again to {@code .}). If a backend performs a second URL decode,
     * {@code %252e%252e} becomes {@code ..}, enabling path traversal.</p>
     *
     * <p>This method performs a case-insensitive scan for {@code %252e} and
     * {@code %252E} (and mixed cases). Following Nginx's approach, we reject
     * any path containing this pattern outright.</p>
     *
     * @param path the path portion of the URI (no query string)
     * @return {@code true} if a double-encoded dot is detected
     */
    static boolean containsDoubleEncodedDot(String path) {
        // We need to find "%252e" (case-insensitive on the final 'e') in the path.
        // %25 is the percent-encoding of '%', so %252e decodes to %2e, then to '.'.
        // The pattern is exactly 5 characters: '%', '2', '5', '2', 'e'/'E'.
        int len = path.length();
     ...
     * @param needle   the lowercase pattern to search for (must be lowercase)
     * @return {@code true} if {@code haystack} contains {@code needle} case-insensitively
     */
    private static boolean containsIgnoreCase(String haystack, String needle) {
        ...
        for (int i = 0; i <= hLen - nLen; i++) {
            for (int j = 0; j < nLen; j++) {
                ...
                char n = needle.charAt(j); // needle is already lowercase
                if (h != n) {
                    continue outer;
                }
            }
     ...
     *
     * <p>This is a faithful transcription of the pseudocode in the RFC. The
     * algorithm processes an input buffer character-by-character, removing "."
     * and ".." path segments to produce a canonical absolute path.</p>
     *
     * <p>The implementation avoids any object allocation beyond the two
     * {@link StringBuilder} instances (no split/join, no regex, no arrays).</p>
     *
     * @param path the path component to normalize (must not contain query string)
     * @return the normalized path with dot segments removed
     */
    static String removeDotSegments(String path) {
        ...
        // Step 1 is just initialization (done above).
        // Steps 2A-2D loop until input is empty.
        while (!input.isEmpty()) {
            // 2A: If the input buffer begins with a prefix of "../" or "./"
            if (startsWith(input, "../")) {
                input.delete(0, 3);
            ...
            //     where "." is a complete path segment
                ...
                input.delete(0, 2); // replace "/./" with "/"
            } else if (equals(input, "/.")) {
                input.replace(0, input.length(), "/");

            // 2C: If the input buffer begins with a prefix of "/../" or "/.."
            //     where ".." is a complete path segment — remove the last segment
            //     from the output buffer
            } else if (startsWith(input, "/../")) {
                input.delete(0, 3); // replace "/../" with "/"
                removeLastSegment(output);
            } else if (equals(input, "/..")) {
                input.replace(0, input.length(), "/");
                removeLastSegment(output);

            // 2D: If the input buffer consists only of "." or ".."
            } else if (equals(input, ".") || equals(input, "..")) {
                input.setLength(0);

            // 2E: Move the first path segment (including initial "/" if any) to output
            } else {
                int segStart = 0;
                // If it starts with '/', include the slash in this segment
                if (input.charAt(0) == '/') {
                    segStart = 1;
                }
                // Find the next '/' after the initial one
                int nextSlash = indexOf(input, '/', segStart);
                if (nextSlash < 0) {
                    // Rest of input is the segment
                    output.append(input);
                    input.setLength(0);
                } else {
    ...
    /** Check if StringBuilder starts with the given prefix (avoids toString allocation). */
    ...
    /** Check if StringBuilder equals the given string exactly (avoids toString allocation). */
    private static boolean equals(StringBuilder sb, String str) {
        if (sb.length() != str.length()) return false;
        return startsWith(sb, str);
    }

    /** Find the first occurrence of ch in sb starting at fromIndex. */
    ...
    /** Remove the last segment from the output buffer (everything from the rightmost "/" to the end). */
