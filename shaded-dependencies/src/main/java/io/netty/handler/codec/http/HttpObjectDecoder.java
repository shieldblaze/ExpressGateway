...
public abstract class HttpObjectDecoder extends ByteToMessageDecoder {
        ...
        switch (currentState) {
            ...
            case READ_VARIABLE_LENGTH_CONTENT: {
                ...
                if (toRead > 0) {
                    ...
                    HttpFrame httpFrame = (HttpFrame) message;
                    out.add(new CustomHttpContent(content, httpFrame.protocol(), httpFrame.id()));
                }
                return;
            }
            case READ_FIXED_LENGTH_CONTENT: {
                ...
                if (chunkSize == 0) {
                    // Read all content.
                    out.add(new CustomLastHttpContent(content, httpFrame.protocol(), httpFrame.id()));
                    resetNow();
                } else {
                    out.add(new CustomHttpContent(content, httpFrame.protocol(), httpFrame.id()));
                }
            ...
            case READ_CHUNKED_CONTENT: {
                ...
                HttpFrame httpFrame = (HttpFrame) message;
                HttpContent chunk = new CustomHttpContent(buffer.readRetainedSlice(toRead), httpFrame.protocol(), httpFrame.id());
                chunkSize -= toRead;
    ...
    private HttpContent invalidChunk(ByteBuf in, Exception cause) {
        ...
        HttpFrame httpFrame = (HttpFrame) message;
        HttpContent chunk = new CustomLastHttpContent(Unpooled.EMPTY_BUFFER, httpFrame.protocol(), httpFrame.id());
    ...
    private LastHttpContent readTrailingHeaders(ByteBuf buffer) {
        ...
        if (line.length() == 0 && trailer == null) {
            ...
            return new CustomLastHttpContent(Unpooled.EMPTY_BUFFER, httpFrame.protocol(), httpFrame.id());
        }

        CharSequence lastHeader = null;
        if (trailer == null) {
            trailer = this.trailer = new CustomLastHttpContent(Unpooled.EMPTY_BUFFER, httpFrame.protocol(), httpFrame.id());
        }
        while (line.length() > 0) {
            char firstChar = line.charAtUnsafe(0);
