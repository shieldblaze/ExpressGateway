...
public class Http2FrameCodec extends Http2ConnectionHandler {
    ...
    public final void userEventTriggered(final ChannelHandlerContext ctx, final Object evt) throws Exception {
        ...
        } else if (evt instanceof UpgradeEvent upgrade) {
            try {
    ...
    private void writeHeadersFrame(final ChannelHandlerContext ctx, Http2HeadersFrame headersFrame,
        ...
        int initialStreamId = headersFrame.stream().id();
        // If Stream ID is valid (i.e. ID >= 0) but Http2Stream is 'null' then the Stream ID
        // is intialized with custom Stream ID.
        //
        // Or
        //
        // If Stream ID is -1 then Stream ID has to be generated automatically by Endpoint counter.
        //
        ...
        if (isStreamIdValid(headersFrame.stream().id())) {
            ...
            promise.tryFailure(new IllegalArgumentException("Invalid Stream ID: " + initialStreamId));
        }
    }

    private void writeHeadersForNewInitializedStream(final ChannelHandlerContext ctx, Http2HeadersFrame headersFrame,
                                                     final ChannelPromise promise) {
        encoder().writeHeaders(ctx, headersFrame.stream().id(), headersFrame.headers(), headersFrame.padding(),
                headersFrame.isEndStream(), promise);
        ...
        if (!promise.isDone()) {
            ...
            promise.addListener(new ChannelFutureListener() {
                ...
                public void operationComplete(ChannelFuture channelFuture) {
                    numBufferedStreams--;
                    handleHeaderFuture(channelFuture, headersFrame.stream().id());
                }
            });
        } else {
            handleHeaderFuture(promise, headersFrame.stream().id());
        }
    }
