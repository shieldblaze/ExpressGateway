...
public class HttpRequestDecoder extends HttpObjectDecoder {
    ...
    protected HttpMessage createMessage(String[] initialLine) {
        HttpVersion httpVersion = HttpVersion.valueOf(initialLine[2]);
        HttpFrame.Protocol protocol;
        if (httpVersion == HttpVersion.HTTP_1_1) {
            protocol = HttpFrame.Protocol.HTTP_1_1;
        } else {
            protocol = HttpFrame.Protocol.HTTP_1_0;
        }
        return new CustomHttpRequest(httpVersion, HttpMethod.valueOf(initialLine[0]), initialLine[1], protocol, RANDOM.nextLong());
    }

    @Override
    protected HttpMessage createInvalidMessage() {
        return new CustomFullHttpRequest(HttpVersion.HTTP_1_0, HttpMethod.GET, "/bad-request", Unpooled.EMPTY_BUFFER,
                HttpFrame.Protocol.HTTP_1_0, RANDOM.nextLong());
    }
