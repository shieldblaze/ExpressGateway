#!/usr/bin/env bash
# S37 LEAD TAKEOVER of the B binding ×3 (verifier's run1 stalled @ 11:22, R9 takeover).
...
cd "$WT" || exit 99
OUT=/home/ubuntu/Code/ExpressGateway/.claude/worktrees/s36-verify/audit/ops/s37-verifyB-lead
mkdir -p "$OUT"
st(){ date -u +%H:%M:%S; }
