...
Status:   Verified-Fixed-Partial(verifier=proto-self, round-6, author-sha=7deeaf3+30f9967+42df990)   <!-- Author-sha=7deeaf3 + 30f9967. H1↔H2 + H2↔H2 trailer pass-through verified at the bridge layer (round-5). Round-6 PROTO-2-19 (commit 42df990) closed the H1-wire-emission gap: bridge fields populated correctly but hyper-1's H1 encoder silently dropped Frame::trailers because chunked-TE + Trailer: declaration were missing on the response head. Fix injects both in `build_h1_response_with_trailers` and drops conflicting Content-Length per RFC 9110 §6.5; proof tests `test_h2_h1_trailers_emitted_on_wire` + `test_h3_h1_trailers_emitted_on_wire` exercise the real hyper H1 encoder and assert the chunked terminator + trailer fields appear on the wire. H3 SURFACE leg still deferred per audit/deferred.md "PROTO-2-12 H3 leg" (lb-quic surface change needed — `H3UpstreamResponse` carries no trailer field today). Forbidden-trailer-name strip remains a follow-up plan. -->
...
Severity: medium
Status:   Verified-Fixed(verifier=proto-self, round-6, author-sha=444668d)   <!-- Round-5 verdict was Verified-Fixed-Partial: validator + SNI capture existed but no proxy hot-path called check_sni_authority. Round-6 PROTO-2-18 (commit 444668d) wired check_sni_authority into both H1Proxy::handle and H2Proxy::handle via serve_connection_with_cancel_sni, with the loopback exception per the sec-r5 caveat (peer.ip().is_loopback() skips enforcement). 421 Misdirected Request is now emitted on every TLS-terminated listener; proof tests at crates/lb-l7/tests/sni_authority_421.rs (real TLS handshake, both 421 and loopback-allow scenarios) PASS. Self-verification — lead may push to sec for adversarial cross-check. -->
