...
Status:   Verified-Fixed-Partial(verifier=sec, round-6-cross-check, author-sha=7deeaf3+30f9967+42df990)   <!-- Author-sha=7deeaf3 + 30f9967. H1↔H2 + H2↔H2 trailer pass-through verified at the bridge layer (round-5). Round-6 PROTO-2-19 (commit 42df990) closed the H1-wire-emission gap: bridge fields populated correctly but hyper-1's H1 encoder silently dropped Frame::trailers because chunked-TE + Trailer: declaration were missing on the response head. Fix injects both in `build_h1_response_with_trailers` and drops conflicting Content-Length per RFC 9110 §6.5; proof tests `test_h2_h1_trailers_emitted_on_wire` + `test_h3_h1_trailers_emitted_on_wire` exercise the real hyper H1 encoder and assert the chunked terminator + trailer fields appear on the wire. Round-6 sec adversarial cross-check (`audit/security/round-6-cross-check.md`) ran four bypass attempts — CRLF in trailer names, name-listed-but-absent, RFC 9110 §6.5.2 forbidden trailers (content-length / transfer-encoding etc.), and request-side TE strip / response-side Trailer add interaction — zero exploitable: HeaderName grammar blocks injection, hyper-1.9.0 encode.rs::is_valid_trailer_field applies the §6.5.2 forbidden filter defensively, and the chunked terminator emits in every branch. H3 SURFACE leg still deferred per audit/deferred.md "PROTO-2-12 H3 leg" (lb-quic surface change needed — `H3UpstreamResponse` carries no trailer field today). Forbidden-trailer-name strip remains a follow-up plan; sec recorded the same gap as INFO-CC-3 (duplicate of the existing rec #4 here). -->
...
Severity: medium
Status:   Verified-Fixed(verifier=sec, round-6-cross-check, author-sha=444668d)   <!-- Round-5 verdict was Verified-Fixed-Partial: validator + SNI capture existed but no proxy hot-path called check_sni_authority. Round-6 PROTO-2-18 (commit 444668d) wired check_sni_authority into both H1Proxy::handle and H2Proxy::handle via serve_connection_with_cancel_sni, with the loopback exception per the sec-r5 caveat (peer.ip().is_loopback() skips enforcement). 421 Misdirected Request is now emitted on every TLS-terminated listener; proof tests at crates/lb-l7/tests/sni_authority_421.rs (real TLS handshake, both 421 and loopback-allow scenarios) PASS. Round-6 sec adversarial cross-check (`audit/security/round-6-cross-check.md`) ran six bypass attempts — case games, trailing-dot, port-in-SNI, empty-authority, v4-mapped-v6 loopback, :authority/Host disagreement precedence — zero exploitable. Precedence verified PROTO-2-01 (400) fires before PROTO-2-18 (421). -->
