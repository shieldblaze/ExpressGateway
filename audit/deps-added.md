...
| `lb-core` | `crates/lb-quic/Cargo.toml` | dep | path (`../lb-core`) | no (lb-quic had no lb-core edge) | ROUND8-L7-16: the H3/QUIC ingress dispatch (`conn_actor::poll_h3`) must run the SAME authority-value predicate the H1/H2 path uses (HAProxy `BUG/MAJOR: http: forbid comma character in authority value`; L7-09's H3 leg). The predicate (`validate` / `AuthorityError`) was hoisted into `lb-core::authority` so there is ONE implementation, not a per-protocol fork. **Cycle check:** `lb-core` is a leaf crate — `cargo tree -p lb-core -e normal` shows ZERO `lb-*` deps, and `lb-l7` (which depends on `lb-quic`) is NOT pulled in, so the alternative (adding `lb-l7` as a dep of `lb-quic`) which WOULD create the `lb-quic ↔ lb-l7` cycle was rejected in favour of this leaf-crate edge. No new external supply-chain surface — `lb-core` is an internal workspace crate already compiled by the `lb` binary. |
| `lb-core` | `crates/lb-l7/Cargo.toml` | dep | path (`../lb-core`) | no (lb-l7 had no lb-core edge) | ROUND8-L7-16: same hoist. `lb_l7::authority` now re-exports `lb_core::authority::{validate, AuthorityError}` and keeps only the hyper `http::Request` wrapper (`validate_request`), so the H1/H2 callsites and their L7-09 proof stay byte-identical with the new H3 path. One-directional: `lb-core` does NOT depend on lb-l7 (no cycle, leaf crate). No new external supply-chain surface — internal workspace crate. |
