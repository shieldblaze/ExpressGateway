...
Severity: critical
Status:   Verified-Fixed(verifier=proto, author-sha=1f7ab4b+fc050b0+82551dc+33edd13+task-38+round8-OPS-04+L4-12)   <!-- Drain ordering at main.rs:1699-1759: set_draining → 1s settle → cancel → listener.abort → quic.shutdown(2s) → shutdown.drain(10s). H2 graceful_shutdown emits canonical two-step GOAWAY (lb-l7 unit test passes). Task-38 closed the three drain test gaps: H1 `serve_connection_with_cancel` + hyper http1 graceful_shutdown wired at H1 / H1s-ALPN accept sites; H2 drain test now generates a self-signed cert + writes a full `[listeners.tls]` block via `rcgen`; H3 `spawn_quic` accepts a CancellationToken cloned from `shutdown.token().child_token()` instead of owning its own. All three drain integration tests pass (`cargo test --test reload_zero_drop`). See audit/protocol/round-5-verifies-rel.md. ROUND-8 OPS-04+L4-12: drain coordinator (`lb_core::Shutdown::run_drain`) replaces the scattered `set_draining → sleep → JoinHandle::abort → drain` pattern with a six-phase enumerated state machine (MarkDraining → ReadinessSettle → ListenerCancel → InFlightDrain → XdpDetach → Total). The 15-case enumeration table (`audit/round-8/fixes/OPS-04-L4-12.md` §B.2) is regression-tested case-by-case in `tests/round8_drain_15case.rs` (15+1 cases, all green). The OPS-03 `shutdown_drain_seconds_{global,listener}` histogram emits per phase via the `MetricsDrainObserver` impl in main.rs. ROUND-8 OPS-03: walked the REL-2-02 recommendation block line-by-line against the emitted /metrics surface — both required REL-2-02 signals are now present: (1) `shutdown_aborted_connections_total` counter (shipped earlier, still bumped from main.rs on InFlightDrain timeout), and (2) `shutdown_drain_seconds` histogram (shipped by the OPS-04+L4-12 coordinator commit as `_global{phase,outcome}` + `_listener{phase,outcome,listener}` with the OPS-03 §A bucket boundaries `[0.1,0.5,1,2.5,5,10,30,60,120,300]`). The histogram was the previously-missing half that had REL-2-02 marked Verified-Fixed on the strength of the ordering alone (coverage-gap Theme 1). OPS-03 adds the `LbShutdownSlow` complementary alert to RUNBOOK.md (p99 InFlightDrain within 20% of the per-listener budget). Status is effectively Verified-Fixed-Complete pending the verifier /metrics walk-through; the histogram is no longer the unfinished half. ROUND-8 OPS-10: the gateway-level 10 s drain budget is correct for short-request HTTP but materially under-budget for gRPC-bidi / WebSocket / SSE / long-poll listeners (Pingora ships EXIT_TIMEOUT=300s); added a per-listener `[[listeners]].drain_timeout_ms` override (validation 100..=300000, inherits [runtime] when absent) plus `effective_drain_timeout_ms`/`effective_drain_jitter_ms` resolvers in lb-config and a `lb_drain_timeout_ms_listener{listener}` build-info gauge. The coordinator's InFlightDrain phase deadline is the max effective per-listener budget; per-connection-await-its-own-listener-budget (OPS-10 §C.3) is a documented div-l7-owned follow-up in the per-conn cancel arm (the accept/serve region OPS-ops must not touch). ROUND-8 OPS-02: a deploy-wide SIGTERM previously made every replica's coordinator fire `token.cancel()` at the same instant (thundering-herd vs the shared upstream LB — Envoy drain_manager_impl.cc; lesson-not-yet-paid-for at our current single-instance test scale); the coordinator now sleeps a per-process random `[0, effective_jitter)` (config `[runtime].drain_jitter_ms` / per-listener override; default = drain_timeout_ms/4) before the InFlightDrain cancel, desynchronising across replicas. RUNBOOK.md gains "Tuning the drain budget", "Drain jitter", LbShutdownSlow and LbShutdownTruncatedStreams. 6 new lb-config unit tests (ops10_*/ops02_*/ops11_*) green; the 15-case drain test stays 16/16 green. Awaiting verifier walk-through against `/metrics` to upgrade the author-sha pin. -->
...
Severity: high
Status:   Verified-Fixed(verifier=proto, author-sha=7108d9e)   <!-- ProbeRegistry AtomicU8 state machine over {Starting,Ready,Draining}; /livez stays 200 across Ready+Draining so K8s does not yank mid-shutdown; /readyz flips to 503 on probes.set_draining() before per-listener cancel (main.rs:1701-1719); /startupz transitions once every listener is bound. JSON contract test updated (6e63f70). See audit/protocol/round-5-verifies-rel.md. ROUND-8 OPS-11: the probe split is unchanged and still Verified-Fixed; OPS-11 found a *parameterisation* gap (not a regression) — `readiness_settle_ms` default was 1000 ms, below the kubelet default `periodSeconds: 10` removal latency, so the lameduck window could close before the upstream observed the 503. OPS-11 raises the default to 11000 ms (one kubelet probe period + 1 s margin) in `lb_config::default_readiness_settle_ms()` and documents the per-upstream tuning table in RUNBOOK.md "Tuning `readiness_settle_ms`". Future work (OPS-11 Option 2): readiness-flip-ack model with `readiness_settle_late_total` counter; tracked separately, not in this bundle. sha=TBD (verifier pin reserved). -->
...
Severity: high
Status:   Proposed-Fix(div-l7, cherry-pick 6253ad9a on prod-readiness/round-4)   <!-- ROUND-8 div-l7 (ROUND8-OPS-06): the recheck-flagged "NO L7 callsite extracts or injects" gap is now CLOSED. `grep extract_parent crates/lb-l7` is no longer zero — crates/lb-l7/src/trace_ctx.rs (RequestTrace::open / inject_upstream) is the first production caller. H1Proxy::handle + H2Proxy::handle open a per-request `lb.l7.request` span (trace_id/span_id/parent_id/http.method/http.target/http.status_code/net.sni/listener) and inject a refreshed child traceparent onto the upstream request incl. the WS-upgrade dial; span carried via `Instrument` (not an Entered guard across .await). Proof: crates/lb-l7/tests/round8_traceparent_propagation.rs (1/1) + round8_ws_upgrade_defer::upstream_receives_child_traceparent + 4 trace_ctx unit tests. Prior status was Verified-Fixed-Partial(verifier=proto, author-sha=1d462c7) — library-only; the deferred proxy wire-in has now landed. OTLP *exporter* knob remains div-ops's house. Verification teammate to confirm + re-grade. -->
