...
## Phase 2 RESULTS — #22 QPACK_DECOMPRESSION_FAILED CLOSED; over-rejection regression FIXED; #21 reclassified

- **#22 ✅ PASS** — a QPACK field-section decode failure (invalid static index, etc.) now closes the connection with `QPACK_DECOMPRESSION_FAILED` (RFC 9204 §2.2), via `FeedError::QpackDecompressionFailed` → `conn.close(true, 0x0200, …)`. +1 unit test.

- **SESSION-INTRODUCED OVER-REJECTION (R14 close-blocker) — FIXED.** The E2 `feed()` change (close on a non-HEADERS frame) was applied to **every** readable stream in `poll_h3`, including the client's **unidirectional control/QPACK streams** — whose stream-TYPE prefix byte `0x00` the request decoder mis-read as a DATA frame → it closed the connection with H3_FRAME_UNEXPECTED. This would have broken **every conformant H3 client** (all open a control stream — h3spec, browsers). It was masked by the lb-quic e2e tests (their minimal clients don't open a control stream) and only surfaced on the FULL h3spec suite (#22 received the H3_FRAME_UNEXPECTED close instead of QPACK_DECOMPRESSION_FAILED). **Fix:** `poll_h3` now applies the request decoder ONLY to client BIDI streams (`sid % 4 == 0`); client UNI streams are drained + discarded (the pre-E2 behaviour, restored safely — and the correct boundary for the carried control-stream findings).
  - **Lesson (recorded):** R5's "re-run the specific case" is necessary but NOT sufficient for H3 — a conformance fix must ALSO be re-run against the FULL suite, because cross-case interactions (here: over-rejecting the control stream every other test opens) are invisible to a single-case run. The two findings that "passed" individually (#11/#21) included one (#21) passing for the WRONG reason (the over-rejection), exposed only by the full suite.

- **#21 RECLASSIFIED → CARRIED.** #11 (DATA before HEADERS) is genuinely fixed (bidi, pre-HEADERS). #21 (CANCEL_PUSH on a request stream) was only "passing" via the over-rejection; h3spec sends the CANCEL_PUSH **after** HEADERS (in the body/DATA phase), which the pre-HEADERS `feed()` guard does not see. Rejecting it needs a **body-phase frame-type guard** in `feed_body` threaded as a connection error — the SAME infrastructure as the carried control-stream findings. Mechanism proven; carried to S23 with #16–20/#24.
