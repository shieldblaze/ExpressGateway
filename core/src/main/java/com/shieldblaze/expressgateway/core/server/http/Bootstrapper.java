    ...
    HTTPConnection newInit(Node node, Channel channel) {
        ...
        bootstrap.handler(new ChannelInitializer<SocketChannel>() {
            ...
            protected void initChannel(SocketChannel ch) {
                ...
                if (httpLoadBalancer.tlsClient() == null) {
                    pipeline.addLast(HTTPUtils.newClientCodec(httpLoadBalancer.getHTTPConfiguration()));
                    ...
                    ALPNHandler alpnHandler = ALPNHandlerBuilder.newBuilder()
                            // HTTP/2 Handlers
                            .withHTTP2ChannelHandler(HTTPUtils.clientH2Handler(httpLoadBalancer.getHTTPConfiguration()))
                            .withHTTP2ChannelHandler(new HTTP2OutboundAdapter())
                            .withHTTP2ChannelHandler(downstreamHandler)
                            // HTTP/1.1 Handlers
                            .withHTTP1ChannelHandler(HTTPUtils.newClientCodec(httpLoadBalancer.getHTTPConfiguration()))
